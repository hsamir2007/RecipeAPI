...
// let link = document.getElementsByClassName(".nav-link .dropdown-item");
...
function getRecipes(rec) {
  ...
  //        method     , Api url
  req.open("GET", `https://forkify-api.herokuapp.com/api/search?q=${rec}`);
  req.send();
  // req.response;
...
function  getDetailsData(drec) {
  ...
  //        method     , Api url
  req.open(
    "GET",`https://forkify-api.herokuapp.com/api/get?rId=${drec}`
  );
  req.send();
  // req.response;
  req.addEventListener("readystatechange", function () {
...
function detailsData() {
  ...
  for (let index = 0; index < detailsDataContainer.length; index++) {
    div += ` <div  class="col-md-4 my-2"><div class="card position-relative d-flex flex-column">
    <a href="${detailsDataContainer[index].source_url}" target="_blank">
             <img  class="img-fluid w-100" src="${detailsDataContainer[index].image_url}" alt="" srcset="">
             </a>
      <div class="card-info my-2">
        <a 
        data-toggle="modal"
        ...
        class="mealsLinks text-decoration-none font-weight-bold">
         ...
         <h5 class="card-text">${detailsDataContainer[index].title}</h5>
...
/**
 * 
 * 1- Asyncronous code (time) غير متزامن
 * setinterval
 * settime out
 * 2- syncronous code  متزامن
 */
